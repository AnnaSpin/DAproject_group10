...
This script intends to solve question 3 of the project
"""
#
...
def plot_series(t,series_data,s,obs_data):
    ...
    names=s['names']
    nseries=len(loc_names_waterlevel)
    fig,ax = plt.subplots(5,1, figsize=(12.8, 12.8), sharex=True)
    plt.suptitle('Comparison of observations and model results at different stations', y=0.92,size=18)
    for i in range(nseries):
        ax[i].plot(t,series_data[i,:], label = 'Model_output') #, colors[i], label = labels[i])
        ntimes=min(len(t),obs_data.shape[1])
        ax[i].plot(t[0:ntimes],obs_data[i,0:ntimes],'k-', label = 'Observation')
        error=-series_data[i,:]+obs_data[i,0:ntimes]
        ax[i].plot(t,error, label = 'Error')
        ax[i].set_title(names[i], x=0.05, y=0, size=12)
        #ax[i].text(left,top,names[i], size=8)
        ax[i].set_ylabel('h [m]', size=14)
...
# =============================================================================
    ...
    ax[4].xaxis.set_minor_locator(mdates.HourLocator(interval=6))
    ax[4].xaxis.set_minor_formatter(mdates.DateFormatter('%H:%M'))
    ax[4].xaxis.grid(False, which='minor')
    ax[4].yaxis.grid(False)
    ax[4].tick_params(axis="x", which="major", pad=14)

    plt.subplots_adjust(hspace=0.0)
    fig.savefig("wave1d_ex3_model_obs_error_comparison.png", dpi =300) 
